...
import {
  BarChart2, Plus, RefreshCw, LogOut, TrendingUp, TrendingDown,
  DollarSign, Wallet, ChevronDown, Trash2, AlertCircle, Loader2,
...
const ars = (v, decimals = 2) =>
  v == null ? '—' : `$ ${Number(v).toLocaleString('es-AR', {minimumFractionDigits: decimals, maximumFractionDigits: decimals})}`

const usd = (v, decimals = 2) =>
  v == null ? '—' : `u$s ${Number(v).toLocaleString('es-AR', {minimumFractionDigits: decimals, maximumFractionDigits: decimals})}`

const pct = (v) =>
  v == null ? '—' : `${v >= 0 ? '+' : ''}${Number(v).toFixed(2)}%`

const qty = (v) =>
  v == null ? '—' : Number(v).toLocaleString('es-AR', {maximumFractionDigits: 4})
...
function AssetBadge({ type }) {
  return (
    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded border ${BADGE[type] || BADGE.OTRO}`}>
      {type}
    </span>
  )
}

// ── Logo (same proxy pattern as existing app) ─────────────────────────────────
  ...
  if (err || !base) {
    return (
      <div className="w-8 h-8 rounded-lg bg-gray-700 flex items-center justify-center flex-shrink-0">
        <span className="text-[10px] font-bold text-gray-400">{(base || '?').slice(0,2)}</span>
      </div>
    )
  }
  return (
...
export default function Dashboard() {
  ...
  const [positions,       setPositions]       = useState(null)   // full response
    ...
    <div className="min-h-screen bg-gray-950 text-gray-100">
      ...
      <header className="sticky top-0 z-30 bg-gray-950/90 backdrop-blur-xl border-b border-gray-800/60">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs text-gray-500 hidden sm:block">{user?.full_name || user?.email}</span>
            <button onClick={logout} className="p-2 text-gray-500 hover:text-gray-300 transition-colors" title="Salir">
              <LogOut size={16} />
            </button>
          </div>
        </div>
        ...
        {showNewPortfolio && (
          <div className="border-t border-gray-800/60 bg-gray-900/80">
            <form onSubmit={createPortfolio} className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-3">
              <input
                autoFocus value={newPortfolioName} onChange={e => setNewPortfolioName(e.target.value)}
              ...
              />
              <button type="submit" className="px-4 py-2 bg-amber-500 hover:bg-amber-400 text-gray-900 font-semibold rounded-lg text-sm">
                Crear
              </button>
              <button type="button" onClick={() => setShowNewPortfolio(false)} className="text-gray-500 hover:text-gray-300">
                <X size={16} />
              </button>
            </form>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 space-y-5">
        ...
        {!loadingList && portfolios.length === 0 && (
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            ...
            <p className="text-gray-400 text-center">No tenés portfolios todavía.<br />
              <button onClick={() => setShowNewPortfolio(true)} className="text-amber-400 hover:text-amber-300 font-medium">
          ...
          <>
            ...
            {positions && (
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <SummaryCard
                  ...
                  value={ars(positions.total_value_ars, 0)}
                  icon={<Wallet size={16} className="text-sky-400" />}
                  sub={`Invertido ${ars(positions.total_cost_ars, 0)}`}
                />
                <SummaryCard
                  label="Ganancia / Pérdida ARS"
                  value={ars(positions.total_pnl_ars, 0)}
                  icon={positions.total_pnl_ars >= 0
                ...
                <SummaryCard
                  ...
                  value={usd(positions.total_value_usd, 0)}
                  icon={<DollarSign size={16} className="text-amber-400" />}
                  sub={`Invertido ${usd(positions.total_cost_usd, 0)}`}
                />
                <SummaryCard
                  label="Ganancia / Pérdida USD"
                  value={usd(positions.total_pnl_usd, 0)}
                  icon={positions.total_pnl_usd >= 0
                    ? <ArrowUpRight size={16} className="text-emerald-400" />
                    : <ArrowDownRight size={16} className="text-red-400" />}
                  positive={positions.total_pnl_usd >= 0}
                  sub={positions.current_mep ? `MEP: $${Number(positions.current_mep).toFixed(0)}` : ''}
                />
              </div>
            )}
            ...
            <div className="flex items-center justify-between">
              <div className="flex gap-1 bg-gray-900 p-1 rounded-xl border border-gray-800">
                {[['positions','Posiciones'],['transactions','Historial']].map(([k,l]) => (
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => { loadPositions(); loadTransactions() }}
                  className="p-2 text-gray-500 hover:text-gray-300 transition-colors"
                  title="Actualizar"
                >
                  <RefreshCw size={16} className={loadingPos ? 'animate-spin' : ''} />
                </button>
                <button
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden">
                {loadingPos ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-20 gap-3 text-gray-500">
                    ...
                    <p className="text-sm">Sin posiciones abiertas. Registrá una operación.</p>
                  </div>
                ) : (
                          ...
                          <tr key={p.ticker} className="hover:bg-gray-800/30 transition-colors">
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <p className="font-mono text-xs font-bold text-sky-400 leading-none mb-1">{p.ticker}</p>
                            ...
                            <td className="px-3 py-3">
                              {p.pnl_usd != null ? (
                                <div className={`text-right ${p.pnl_usd >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                                  <p className="font-mono text-sm font-semibold">{usd(p.pnl_usd)}</p>
                                  <p className="text-[10px] font-bold">{pct(p.pnl_pct_usd)}</p>
                                </div>
                              ) : <p className="text-right text-gray-600 text-xs">sin MEP</p>}
                            </td>
                          </tr>
                        ))}
                      </tbody>
                    ...
                    <table className="w-full text-left border-collapse">
                      ...
                      <tbody className="divide-y divide-gray-800/60">
                        {transactions.map(t => {
                          ...
                          const totalUsd = t.mep_rate ? totalArs / t.mep_rate : null
                          const isBuy    = t.transaction_type === 'COMPRA'
                          return (
                            <tr key={t.id} className="hover:bg-gray-800/30 transition-colors text-sm">
                              ...
                              <td className="px-3 py-3 font-mono text-xs font-bold text-sky-400">{t.ticker}</td>
                              <td className="px-3 py-3 text-right font-mono text-gray-300">{qty(t.quantity)}</td>
                              <td className="px-3 py-3 text-right font-mono text-gray-300">{ars(t.unit_price_ars)}</td>
                              <td className="px-3 py-3 text-right font-mono font-semibold text-white">{ars(totalArs, 0)}</td>
                              <td className="px-3 py-3 text-right font-mono text-gray-400 text-xs">
                                {t.mep_rate ? `$${Number(t.mep_rate).toFixed(0)}` : '—'}
                              </td>
      ...
      {showAddTx && (
        <AddTransaction
          ...
          onSuccess={() => { loadPositions(); loadTransactions() }}
        />
      )}
    </div>
  )
